...
function wrap_rocdbg_fun!(ctx::AbstractContext, cursor::CLFunctionDecl)
    ...
    signature = efunsig(Symbol(spelling(cursor)[12:end]), arg_names, arg_reps)
    body = eccall(func_name, Symbol("librocm-dbgapi"), ret_type, arg_names, arg_reps)
    push!(ctx.api_buffer, Expr(:function, signature, Expr(:block, body)))

    return ctx
end

function wrap_rocdbg_struct!(ctx::AbstractContext, cursor::CLStructDecl)
